...
import pt.tooyummytogo.padroesStrategies.CatPesquisa;
import pt.tooyummytogo.padroesStrategies.DefinePeriodoStrategy;
import pt.tooyummytogo.padroesStrategies.DefineRaioStrategy;
import pt.tooyummytogo.padroesStrategies.IndicaLocalizacaoStrategy;
import pt.tooyummytogo.padroesStrategies.PesquisaComerciantes;
/**
...
public class CatUtilizadores {
	...
	private CatPesquisa catPesquisa = new CatPesquisa();
	 ...
	 */
	public List<ComercianteInfo> indicaLocalizacao(PosicaoCoordenadas coordinate) {
		
		String estrategia = "pt.tooyummytogo.padroesStrategies.IndicaLocalizacaoStrategy";
		Optional<Object> s = createInstanceOfClass(estrategia);
		s.ifPresent((o) -> {
			listaComerciantes.clear();
			PesquisaComerciantes st = (PesquisaComerciantes) o;
			catPesquisa.setMapa(catalogoComerciantes);
			catPesquisa.setCoordenadasConsumidor(coordinate);
			for(ComercianteInfo c : catPesquisa.getListaComerciantes(st)) {
				listaComerciantes.add(c);
			}
		});
	...
	public List<ComercianteInfo> defineRaio(int i) {
		...
		String estrategia = "pt.tooyummytogo.padroesStrategies.DefineRaio";
		Optional<Object> s = createInstanceOfClass(estrategia);
		s.ifPresent((o) -> {
			listaComerciantes.clear();
			PesquisaComerciantes st = (PesquisaComerciantes) o;
			catPesquisa.setNovoRaio(i);
			for(ComercianteInfo c : catPesquisa.getListaComerciantes(st)) {
				listaComerciantes.add(c);
			}
	...
	public List<ComercianteInfo> definePeriodo(LocalDateTime now, LocalDateTime plusHours) { //1.2 UC6 definePeriodo
		...
		String estrategia = "pt.tooyummytogo.padroesStrategies.DefinePeriodoStrategy";
		Optional<Object> s = createInstanceOfClass(estrategia);
		s.ifPresent((o) -> {
			listaComerciantes.clear();
			PesquisaComerciantes st = (PesquisaComerciantes) o;
			catPesquisa.setHoras(now, plusHours);
			for(ComercianteInfo c : catPesquisa.getListaComerciantes(st)) {
				listaComerciantes.add(c);
			}
	...
	/**
	 * Metodo que retorna uma instancia de da class pedida
	 * @param nomeDaClasse - o nome da classe que se pretende retornar uma instancia
	 * @return uma instancia da classe pedida
	 */
	private static Optional<Object> createInstanceOfClass(String nomeDaClasse) {
  ...
  //método para testar JUnit
    public Consumidor getConsumidorCorrente(String nome) {
        return catalogoConsumidores.get(nome);
    }
}
